...
parser.add_argument('--reward_type', type=str, default='sparse',
                    help='define the reward type (default: sparse)')
...
parser.add_argument('--alpha', type=float, default=0.01,
                    help='importance of learning the intrinsic cur reward')
parser.add_argument('--beta', type=float, default=1000,
                    help='importance of learning the intrinsic mi reward')
...
if __name__ == '__main__':
    ...
    SAVEPATH = os.getcwd() + '/save/scmemi_'+ args.reward_type +'/mario_a3c_params.pkl'
    if not os.path.exists(os.getcwd() + '/save/scmemi_'+ args.reward_type):
        os.makedirs(os.getcwd() + '/save/scmemi_'+ args.reward_type)
    ...
    optimizer = SharedAdam(list(shared_model.parameters()) + list(shared_scme.parameters()), lr=args.lr)
...
#     optimizer_icm.share_memory()
    ...
    for rank in range(0, num_procs):
        if rank < sample_val:                           # select random action
            p = mp.Process(target=train, args=(rank, args, shared_model, shared_scme, counter, lock, optimizer))
        else:                                           # select best action
            p = mp.Process(target=train, args=(rank, args, shared_model, shared_scme, counter, lock, optimizer, False))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
