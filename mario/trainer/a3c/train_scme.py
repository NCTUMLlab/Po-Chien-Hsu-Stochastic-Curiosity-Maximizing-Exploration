...
def loss_function(recon_x, x, mu, logvar):
    # reconstruction loss
    BCE = F.mse_loss(recon_x.view(-1, 4*42*42), x.view(-1, 4*42*42), reduction = "sum")
...
def train(rank, args, shared_model, shared_scme, counter, lock, optimizer=None, select_sample=True):
    ...
    savefile = os.getcwd() + '/save/scme_'+ args.reward_type +'/train_reward.csv'
    saveweights = os.getcwd() + '/save/scme_'+ args.reward_type +'/mario_a3c_params.pkl'
    ...
    for num_iter in count():
        ...
        for step in range(args.num_steps):
            ...
            pred_z, actual_z, xt1_hat, xt1, xt1_mu, xt1_logvar = scme_model((state_inp, next_state_inp, action_one_hot))
...
#            env.locked_levels = [False] + [True] * 31
        ...
        for i in reversed(range(len(rewards))):
            ...
            scme_loss = cur_losses[i] + 0.01 * vae_losses[i] 
            # - args.beta*mi_losses[i]
...
def test(rank, args, shared_model, counter):
    ...
    savefile = os.getcwd() + '/save/scme_'+ args.reward_type +'/mario_curves.csv'
