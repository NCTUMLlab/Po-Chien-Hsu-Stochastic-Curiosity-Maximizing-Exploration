    ...
    def train(self):
        ...
        # if self.samplecount >= self.capacity and self.episodecount % 5 == 0:
            ...
            for _ in range(self.train_iters_per_episode):
                ...
                #train
                if self.intrinsic_reward_method == 'vime':
                    total = self.vime_model.train(np.concatenate(np.array(s2_batch), axis=0).tolist(), np.concatenate(np.array(s_batch), axis=0).tolist(), a_batch_one_hot)
                    self.vime_model.save_model('model/vime_model/' + self.out_policy_file)

                elif self.intrinsic_reward_method == 'cme':
                    total = self.cme_model.train(np.concatenate(np.array(s2_batch), axis=0).tolist(), np.concatenate(np.array(s_batch), axis=0).tolist(), a_batch_one_hot)
                    self.cme_model.save_model('model/cme_model/' + self.out_policy_file)

                elif self.intrinsic_reward_method == 'scme':
                    total = self.scme_model.train(np.concatenate(np.array(s2_batch), axis=0).tolist(), np.concatenate(np.array(s_batch), axis=0).tolist(), a_batch_one_hot)
                    self.scme_model.save_model('model/scme_model/' + self.out_policy_file)    
                #improvement==================================
