...
class DQNPolicy(Policy.Policy):
    ...
    def __init__(self, in_policy_file, out_policy_file, domainString='CamRestaurants', is_training=False, action_names=None):
        ...
        if utils.Settings.config.has_option('scme', 'method'):
            self.intrinsic_reward_method = utils.Settings.config.get('scme', 'method')
        #improvement==================================
    ...
    def train(self):
        ...
        if self.samplecount >= self.minibatch_size * 10 and self.episodecount % self.training_frequency == 0:
            ...
            #train
            if self.intrinsic_reward_method == 'vime':
                total = self.vime_model.train(s2_batch, s_batch, a_batch_one_hot)
                self.vime_model.save_model('model/vime_model/' + self.out_policy_file)

            elif self.intrinsic_reward_method == 'cme':
                total = self.cme_model.train(s2_batch, s_batch, a_batch_one_hot)
                self.cme_model.save_model('model/cme_model/' + self.out_policy_file)

            elif self.intrinsic_reward_method == 'scme':
                total = self.scme_model.train(s2_batch, s_batch, a_batch_one_hot)
