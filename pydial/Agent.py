...
class DialogueAgent(object):
    ...
    def __init__(self, agent_id='Smith', hub_id='dialogueserver'):        
        ...
        self.domains = None
        if Settings.config.has_option('GENERAL', 'domains'):
            self.domains = Settings.config.get('GENERAL', 'domains')
        self.action_names = []  # hardcoded to include slots for specific actions (request, confirm, select)
        if self.domains == 'CamRestaurants':
            self.action_names += ["request(food", "request(area", "request(pricerange",
                                  ...
                                  "reqmore",
                                  "restart"]
        elif self.domains == 'SFRestaurants':
            self.action_names += ["request(food", "request(area", "request(pricerange", "request(near", "request(goodformeal", "request(allowedforkids",
                                  "confirm(food", "confirm(area", "confirm(pricerange", "confirm(near", "confirm(goodformeal", "confirm(allowedforkids",
                                  "select(food", "select(area", "select(pricerange", "select(near", "select(goodformeal", "select(allowedforkids",
                                  "inform",
                                  "inform_byname",
                                  "inform_alternatives",
                                  "bye",
                                  ...
                                  "reqmore",
                                  "restart"]
        elif self.domains == 'Laptops11':    
            self.action_names += ["request(family", "request(utility", "request(pricerange", "request(driverange", "request(weightrange", "request(batteryrating",
                                  "request(platform", "request(processorclass", "request(sysmemory", "request(warranty", "request(isforbusinesscomputing",
                                  "confirm(family", "confirm(utility", "confirm(pricerange", "confirm(driverange", "confirm(weightrange", "confirm(batteryrating",
                                  "confirm(platform", "confirm(processorclass", "confirm(sysmemory", "confirm(warranty", "confirm(isforbusinesscomputing",
                                  "select(family", "select(utility", "select(pricerange", "select(driverange", "select(weightrange", "select(batteryrating",
                                  "select(platform", "select(processorclass", "select(sysmemory", "select(warranty", "select(isforbusinesscomputing",
                                  "inform",
                                  "inform_byname",
                                  "inform_alternatives",
                                  "bye",
